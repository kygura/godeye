> Reference only — ported from `gev-wip-travel-layer` (`docs/design/meridian-merge.md`),
> the WIP scaffold this build's Trips layer is based on. Superseded in two ways: the city
> dataset below (`public/data/travel/cities.json` via `scripts/build-travel-cities.mjs`) was
> **not** ported — Trips reads `src/data/local_data/city_intel/` instead, see
> `docs/cockpit/SPEC.md` §3.2 — and the command-palette/ergonomics redesign below was not
> built in this pass. Kept for the parts that did port: geo/seasonality/citySearch/tripStore
> and the Trips globe layer.
