 ...
 * @returns {{enter: Function, exit: Function, isActive: () => boolean,
 *   openTravelBriefing: (place: {name?: string, lat: number, lon: number}) => Promise<void>,
 *   destroy: Function}}
 */
export function createTravelMode({
  ...
  if (!panel || !toggle)
    return { destroy() {}, openTravelBriefing: async () => {} };
  ...
  /** Shared tail of setDestination/openTravelBriefing once a place is resolved. */
  function applyDestination(
    { lat, lon, label },
    { fly = true, urlDest = label } = {},
  ) {
    dest = { lat, lon, label };
    input.value = label;
    panel.dataset.ready = '';
    ...
    history.replaceState(
      history.state,
      '',
      travelUrl(location.href, { active: true, dest: urlDest }),
    );
  ...
  async function setDestination(query, { fly = true } = {}) {
    ...
    applyDestination(
      { lat: place.lat, lon: place.lng, label },
      { fly, urlDest: text },
    );
  }

  /**
   * Open the briefing for an already-resolved place, skipping the geocoder —
   * the seam for a caller that already has coordinates (e.g. a City Intel
   * scorecard's "Brief me" button). Falls back to the text geocode path
   * (`setDestination`) when lat/lon are not given.
   *
   * Reachable from app wiring via the object `createTravelMode()` returns:
   * `src/app/tools.js` stores it as `travelMode` (both on the function's
   * return value, exposed through `application.getComponents().tools
   * .travelMode`, and on the `window.__godsEyeView.travelMode` debug handle).
   *
   * @param {{name?: string, lat: number, lon: number}} place
   * @returns {Promise<void>}
   */
  async function openTravelBriefing({ name, lat, lon } = {}) {
    if (!Number.isFinite(lat) || !Number.isFinite(lon))
      return setDestination(name);
    briefing?.abort();
    flightController?.abort();
    ...
    applyDestination({
      lat,
      lon,
      label: name || `${lat.toFixed(2)}, ${lon.toFixed(2)}`,
    });
  }
