...
export const COMFORT_LABELS = ['HARSH', 'THIN', 'MILD', 'FAIR', 'GOOD', 'PRIME'];
...
export function comfortScore(tempC, precipMm) {
  ...
  const penalty = precipMm <= 30 ? 0 : precipMm >= 250 ? 35 : (35 * (precipMm - 30)) / 220;
...
export function bestWindow(months) {
  ...
  if (cand.every(Boolean))
    return { startMonth: 0, endMonth: 11, peakMonth: globalPeak, isWrap: false };
  const runs = [];
  for (let i = 0; i < 12; i++) {
  ...
  return { startMonth: best.start, endMonth, peakMonth, isWrap: endMonth < best.start };
}
