...
test('cyclones are unavailable outside NHC basins and escalate by nearest approach', () => {
  ...
  assert.equal(
    assessCyclones({ unavailable: true, storms: [] }, LISBON).status,
    'unavailable',
  );
  ...
  assert.equal(
    assessCyclones({ storms: [storm(450)] }, LISBON).status,
    'elevated',
  );
  assert.equal(assessCyclones({ storms: [storm(200)] }, LISBON).status, 'high');
  const forecastNear = storm(900, {
    forecastPoints: [
      { position: { latitude: north(250).lat, longitude: LISBON.lon } },
    ],
  });
  assert.equal(
    assessCyclones({ storms: [forecastNear] }, LISBON).status,
    'high',
  );
  const box = (d) => [
  ...
  const coneOver = storm(2000, {
    cone: { type: 'Polygon', coordinates: [box(20)] },
  });
  assert.equal(
    assessCyclones({ storms: [coneOver] }, LISBON).items[0].distanceKm,
    0,
  );
});
...
test('overpass elements map to help points and group by kind', () => {
  assert.deepEqual(
    toHelpPoint({
      ...
      id: 7,
      center: { lat: 1, lon: 2 },
      tags: { amenity: 'hospital', name: 'São José' },
    }),
    {
      id: 'way/7',
      kind: 'medical',
  ...
  assert.equal(
    toHelpPoint({ id: 1, lat: 1, lon: 2, tags: { office: 'diplomatic' } }).kind,
    'embassy',
  );
  assert.equal(
    toHelpPoint({ id: 1, lat: 1, lon: 2, tags: { office: 'diplomatic' } }).name,
    'Embassy',
  );
  assert.equal(
    toHelpPoint({ id: 1, lat: 1, lon: 2, tags: { amenity: 'bar' } }),
    null,
  );
...
test('help query stays inside the proxy bounds', () => {
  ...
  assert.equal(
    [...query.matchAll(/around:10000,38\.72230,-9\.13930/g)].length,
    2,
  );
...
test('overall risk is the worst rated section and names the driver', () => {
  ...
  assert.deepEqual(overall.drivers, [
    { id: 'seismic', label: 'Seismic', reason: '2 quakes' },
  ]);
  assert.deepEqual(overall.notChecked, ['Cyclones', 'Fires']);
  assert.equal(
    overallRisk({ fires: { status: 'unavailable' } }).status,
    'unavailable',
  );
  assert.equal(
    overallRisk({ weather: { status: 'loading' } }).status,
    'unavailable',
  );
});
...
test('travel URL params parse and rewrite without touching the hash', () => {
  assert.deepEqual(parseTravelParams('?mode=travel&dest=%20Lisbon%20'), {
    active: true,
    dest: 'Lisbon',
  });
  assert.deepEqual(parseTravelParams('?dest=Lisbon'), {
    active: false,
    dest: 'Lisbon',
  });
  ...
  assert.equal(
    travelUrl(href, { active: true, dest: 'São Paulo' }),
    '/?keep=1&mode=travel&dest=S%C3%A3o+Paulo#style=normal',
  );
  assert.equal(
    travelUrl('http://x.test/?keep=1&mode=travel&dest=Lisbon#h', {
      active: false,
    }),
    '/?keep=1#h',
  );
