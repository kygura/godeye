...
import { flightPath, estimateFlightHours, haversineKm } from '../../travel/geo.js';
...
export function tripLegs(trip, hubs = FLIGHT_HUBS) {
  ...
  for (let i = 1; i < nodes.length; i++) {
    ...
    legs.push({ from: nodes[i - 1], to: nodes[i], ...path, hours: estimateFlightHours(path.distanceKm) });
  }
  return legs;
}
    ...
    chips: [
      {
        ...
        title: nodes < 2 ? 'Add two or more stops first' : 'Fly the camera along the trip',
        params: { fly: true },
      },
      {
        id: 'clear',
...
export function createTripsLayer({ store, camera = null } = {}) {
  if (typeof store?.subscribe !== 'function') throw new TypeError('Trips require a trip store');
  let _viewer = null;
  ...
  function fly() {
    const trip = store.getActiveTrip();
    if (!camera?.flyRoute || !_viewer || !trip || trip.nodes.length < 2) return false;
    camera.initCameraVerbs?.(_viewer);
    const path = tripLegs(trip).flatMap((leg) => leg.segments.flat()).map(([lon, lat]) => ({ lon, lat, height: 0 }));
    ...
    _flying = true;
    repaintRow();
    const done = () => { _flying = false; repaintRow(); };
  ...
  return {
    ...
    getStats() {
      ...
      return { count: trip?.nodes.length || 0, enabled: _enabled, distanceKm: km,
        detail: trip ? `${trip.name} · ${km.toLocaleString()} km` : 'No active trip' };
    },
    getRowControls() {
