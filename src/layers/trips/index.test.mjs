...
const trip = { name: 'T', color: '#fff', nodes: [
...
test('a London-Auckland leg is hub-routed and FLY needs two stops', () => {
  ...
  assert.equal(tripsRowControls({ trip: { nodes: [trip.nodes[0]] } }).chips[0].disabled, true);
  assert.equal(tripsRowControls({ trip, flying: false }).chips[0].disabled, false);
